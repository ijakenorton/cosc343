...
def fitness_score(trial, code, guesses, slots=5):
    '''
...
def genetic_evolution(popsize, generations, costfitness,
                      eliteratio=ELITE_RATIO, slots=5):
    '''
    ...
    while len(chosen_ones) <= popsize and h <= generations:
        ...
        for code in eligibles:
            if code in chosen_ones:
                ...
                chosen_ones.append([random.randint(1, len(COLORS))
                                   for i in range(slots)])
        ...
        j = len(eligibles)
        while j < popsize:
            population.append([random.randint(1, len(COLORS))
                              for i in range(slots)])
            j = j + 1
...
def main():
    ...
    def scoref(trial):
        return fitness_score(trial, code, guesses, slots)

    
    turns = 0
    for i in range(0,100):
        TOGUESS = [random.randint(1, slots) for _ in range(slots)]

        random.seed(os.urandom(32))
        G1 = [1, 1, 2, 3,1]  # initial guess
        code = G1
        turn = 1
    ...
    # List of all tried guesses Gi
        ...
        while result != (slots, 0):
            eligibles = genetic_evolution(
                MAX_POP_SIZE, MAX_GENERATIONS, scoref, slots=slots)
            ...
            while len(eligibles) == 0:
                ...
                # print(guesses)
                eligibles = genetic_evolution(
                    MAX_POP_SIZE*2, MAX_GENERATIONS/2, scoref, slots=slots)
